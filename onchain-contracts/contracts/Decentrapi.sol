...
contract decentrapi is Ownable {
    ...
    event NodeRemoved(address indexed sender, uint nodeId);
    ...
    function addNode(string memory _url, uint _port) public onlyOwner returns(bool success){
        ...
        require(keccak256(abi.encodePacked(nodes[hashedUrl].url)) != keccak256(abi.encodePacked(_url)), "addNode: Node already added");
    ...
    function removeNode(uint nodeId) public onlyOwner returns(bool success){
        require(nodeId < nodeIndex.length, "removeNode: nodeId does not exist");

        bytes32 storedHash = nodeIndex[nodeId];

        bytes32 tmpHash         = nodeIndex[nodeIndex.length-1];
        nodeIndex[nodeId]       = tmpHash;
        nodes[tmpHash].index    = nodeId;
        nodeIndex.pop();

        nodes[storedHash].index  = 0;
        nodes[storedHash].port   = 0;
        nodes[storedHash].url    = "";
